...
/-- The universal property states that a functor `L : C ⥤ D` inverts the morphisms in `W`, every functor
`F : C ⥤ E` (for a fixed category `E`) inverting `W` admits a unique factorisation through `L`. -/
